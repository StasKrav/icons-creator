...
let state = {
  ...
  snap: true,
  gridSize: 32,
  gridMode: "square", // 'square', 'triangular', 'dots'
  triGridSize: 32,
  showHandles: false,
  lastCreatedCurveIdx: -1,
  ghostStroke: true,
  // НОВЫЕ ПОЛЯ ДЛЯ ЗАЛИВКИ
  fillType: "none", // 'none', 'solid', 'gradient'
  fillColor: "#3b82f6",
  fillGradient: {
...
// Пресеты для разных размеров иконок (канвас 512×512)
const iconPresets = {
  favicon: {
    // 16×16px
    targetSize: 16,
  ...
  high128: {
    ...
    blocksY: 128,
    minThickness: 4,
    description: "Высокое разрешение (128×128)",
  },
  ultra256: {
    // 256×256px
    targetSize: 256,
  ...
  full512: {
    ...
    blocksY: 512,
    minThickness: 1,
    description: "Полное разрешение (512×512)",
  },
};
...
function setIconMode(mode) {
  ...
  // Ограничиваем толщину линии
  if (thickness) {
    thickness.min = preset.minThickness;

    // Если текущая толщина меньше минимальной - поднимаем
  ...
  console.log(`🎯 Режим: ${preset.description}`);
  console.log(`📐 Сетка: ${preset.gridSize}px`);
  console.log(`🔢 Блоков: ${preset.blocksX}×${preset.blocksY}`);
  console.log(`✏️ Мин. толщина: ${preset.minThickness}px`);
}
...
// Функция обновления панели настроек из выбранного объекта
function updatePanelFromSelected() {
  if (state.selectedPathIdx !== -1 && state.paths[state.selectedPathIdx]) {
    ...
    if (fillGradientColor1 && selectedPath.fillGradient) {
      state.fillGradient = JSON.parse(
        JSON.stringify(selectedPath.fillGradient),
      );
      ...
      if (fillAngleVal)
        fillAngleVal.textContent = `${state.fillGradient.angle}°`;
    }
...
function snapXY(x, y) {
  ...
  if (state.gridMode === "triangular") {
    return snapToTriangular(x, y);
  } else if (state.gridMode === "isometric") {
    return snapToIsometric(x, y);
  }
...
// Привязка к изометрической сетке
function snapToIsometric(x, y) {
  const size = state.triGridSize;
  const angleRad = ((state.isoAngle || 30) * Math.PI) / 180;

  // Сетка рисуется из центра (cx, cy).
  // Ось X (dir = cos(a), sin(a)):  линия i проходит через cx + i*size*(-sin(a), cos(a))
  // Ось Y (dir = cos(a), -sin(a)): линия i проходит через cx + i*size*(sin(a), cos(a))
  // Ось Z (dir = 0, 1):            линия i проходит через cx + i*size*(1, 0)
  //
  // Узел сетки = пересечение двух линий из разных семейств.
  // Находим ближайший узел прямым перебором.
  ...
  // Максимальный индекс линии, который может быть виден на канвасе
  ...
  // Перебираем все видимые линии оси X
  for (let iX = -maxI; iX <= maxI; iX++) {
    // Точка на линии оси X: центр + iX * size * perpX
    // perpX = (-sin(a), cos(a))
    const px = cx + iX * size * -sinA;
    const py = cy + iX * size * cosA;

    // Перебираем все видимые линии оси Y
    for (let iY = -maxI; iY <= maxI; iY++) {
      // Точка на линии оси Y: центр + iY * size * perpY
      // perpY = (sin(a), cos(a))
      const qx = cx + iY * size * sinA;
      const qy = cy + iY * size * cosA;

      // Пересечение линий iX (оси X) и iY (оси Y):
      // Линия X проходит через (px, py) в направлении (cosA, sinA)
      // Линия Y проходит через (qx, qy) в направлении (cosA, -sinA)
      //
      // Параметрически:
      //   X: (px + t*cosA, py + t*sinA)
      //   Y: (qx + s*cosA, qy - s*sinA)
      //
      // Приравниваем: px + t*cosA = qx + s*cosA
      //              py + t*sinA = qy - s*sinA
      //
      // Из первого: t*cosA - s*cosA = qx - px → (t-s)*cosA = qx - px
      // Из второго: t*sinA + s*sinA = qy - py → (t+s)*sinA = qy - py
      //
      // t = ((qx-px)/cosA + (qy-py)/sinA) / 2
      // s = ((qy-py)/sinA - (qx-px)/cosA) / 2
    ...
    // Пересечение оси X и оси Z:
    // Линия Z iZ проходит через (cx + iZ*size, cy) в направлении (0, 1)
    for (let iZ = -maxI; iZ <= maxI; iZ++) {
      const rz = cx + iZ * size;

      // Пересечение: (px + t*cosA, py + t*sinA) = (rz, cy + s)
      // px + t*cosA = rz → t = (rz - px) / cosA
      // py + t*sinA = cy + s → s = py + t*sinA - cy

      if (Math.abs(cosA) > 1e-10) {
        const tz = (rz - px) / cosA;
        const nzx = px + tz * cosA;
        const nzy = py + tz * sinA;
  ...
  for (let iY = -maxI; iY <= maxI; iY++) {
    ...
    for (let iZ = -maxI; iZ <= maxI; iZ++) {
      ...
      // Пересечение: (qx + s*cosA, qy - s*sinA) = (rz, cy + t)
      // qx + s*cosA = rz → s = (rz - qx) / cosA
      if (Math.abs(cosA) > 1e-10) {
        const sz = (rz - qx) / cosA;
        const nzx = qx + sz * cosA;
        const nzy = qy - sz * sinA;
...
// Получение позиции мыши (исправленная версия)
function getMousePos(e) {
  ...
  if (state.gridMode === "triangular") {
    return snapToTriangular(rawX, rawY);
  } else if (state.gridMode === "isometric") {
    return snapToIsometric(rawX, rawY);
  }
...
function onMouseDown(e) {
  ...
  // Остальные инструменты (линия, прямоугольник, круг)
...
function onMouseMove(e) {
  ...
  if (
    ...
    // Временные ручки для предпросмотра
  ...
  if (state.isDragging && state.dragType === "handle") {
    ...
    if (path.tool === "curve" && path.points.length === 4) {
      // Для новой кривой с двумя ручками
  ...
  if (state.isDragging && state.dragType === "point") {
    ...
    // Если это прямоугольник, обновляем связанные точки
  ...
  if (state.isDragging && state.dragType === "path") {
    ...
    state.paths[pathIdx].points = state.paths[pathIdx].points.map((point) => ({
      x: point.x + dx,
      y: point.y + dy,
    }));
  ...
  if (state.currentPath.length === 1) {
    ...
    if (state.tool === "line") {
      newPath = {
        tool: "line",
        points: [
          { x: state.currentPath[0].x, y: state.currentPath[0].y },
          { x: pos.x, y: pos.y },
        ],
      ...
      newPath = {
        ...
        points: [
          { x: state.currentPath[0].x, y: state.currentPath[0].y },
          { x: pos.x, y: state.currentPath[0].y },
          { x: pos.x, y: pos.y },
          { x: state.currentPath[0].x, y: pos.y },
        ],
        thickness: state.thickness,
        color: state.color,
      };
    ...
    if (newPath) {
      ...
      // Принудительная перерисовка с очисткой
...
function updateRectanglePoints(rect, changedPointIdx, newPos) {
  ...
  if (changedPointIdx === 0) {
    points[1].y = newPos.y;
    points[3].x = newPos.x;
  } else if (changedPointIdx === 1) {
    points[0].y = newPos.y;
    points[2].x = newPos.x;
  } else if (changedPointIdx === 2) {
    points[1].x = newPos.x;
    points[3].y = newPos.y;
  } else if (changedPointIdx === 3) {
    points[0].x = newPos.x;
    points[2].y = newPos.y;
  }
}
...
function findPathAt(x, y) {
  // Идём с конца (чтобы выбирать верхние фигуры)
  for (let i = state.paths.length - 1; i >= 0; i--) {
    const path = state.paths[i];
    if (isPointOnPath(x, y, path)) {
      return i;
    }
  }
  return -1;
...
function isPointOnPath(x, y, path) {
  ...
  // Размер "зоны захвата" - чуть больше толщины линии
  ...
  if (path.tool === "rectangle" && path.points.length === 4) {
    ...
    // Проверяем, внутри ли курсор (с учётом запаса)
    if (
      x >= minX - hitMargin &&
      x <= maxX + hitMargin &&
      y >= minY - hitMargin &&
    ...
    // Проверяем, рядом ли с окружностью
    if (Math.abs(dist - radius) <= hitMargin) {
      return true;
    }
    // Или внутри круга (для залитых фигур)
    const hasFill = path.fillType !== "none";
    if (hasFill && dist <= radius + hitMargin) {
      return true;
    }
  } else if (path.tool === "line" && path.points.length === 2) {
    const dist = distanceToLine(x, y, path.points[0], path.points[1]);
    if (dist <= hitMargin) {
      // Дополнительно проверяем, что точка проекции между концами линии
      const dot1 =
        (x - path.points[0].x) * (path.points[1].x - path.points[0].x) +
        (y - path.points[0].y) * (path.points[1].y - path.points[0].y);
      const dot2 =
        (x - path.points[1].x) * (path.points[0].x - path.points[1].x) +
        (y - path.points[1].y) * (path.points[0].y - path.points[1].y);
      if (dot1 >= -hitMargin && dot2 >= -hitMargin) {
        return true;
      }
    }
  } else if (path.tool === "curve" && path.points.length === 4) {
    // Проверяем близость к кривой
    for (let t = 0; t <= 1; t += 0.05) {
      const t1 = 1 - t;
      const px =
        t1 * t1 * t1 * path.points[0].x +
        3 * t1 * t1 * t * path.points[1].x +
        3 * t1 * t * t * path.points[2].x +
        t * t * t * path.points[3].x;
      const py =
        t1 * t1 * t1 * path.points[0].y +
        3 * t1 * t1 * t * path.points[1].y +
        3 * t1 * t * t * path.points[2].y +
        t * t * t * path.points[3].y;
      if (Math.hypot(px - x, py - y) <= hitMargin) {
        return true;
      }
    }
  }
...
function isPointNearPath(x, y, path) {
  ...
  if (path.tool === "curve" && path.points.length === 4) {
    // Проверяем близость к кривой Безье (более точная проверка)
    for (let t = 0; t <= 1; t += 0.01) {
      // Формула Безье с двумя ручками
      const t1 = 1 - t;
      const point = {
        x:
          t1 * t1 * t1 * path.points[0].x +
    ...
    // Проверяем ручки
    if (
      Math.hypot(path.points[1].x - x, path.points[1].y - y) < threshold + 5 ||
      Math.hypot(path.points[2].x - x, path.points[2].y - y) < threshold + 5
    ) {
      return true;
    }
  } else if (path.tool === "line" && path.points.length === 2) {
    ...
    const adjustedThreshold =
      lineLength < 30 ? sizeThreshold + 5 : sizeThreshold;
    ...
    if (maxX - minX < 30 && maxY - minY < 30) {
      if (
        x >= minX - sizeThreshold &&
        x <= maxX + sizeThreshold &&
        y >= minY - sizeThreshold &&
        y <= maxY + sizeThreshold
      ) {
        return true;
      }
    }
    ...
    if (Math.abs(distance - radius) < adjustedThreshold) {
      return true;
    }

    if (radius < 5 && distance < 15) {
      return true;
    }
  }
...
// Проверка, имеет ли фигура видимое представление (заливка или контур)
...
function draw() {
  ...
  // ========== РУЧКИ И ТОЧКИ (оставляем как было) ==========
  state.paths.forEach((path, idx) => {
    ...
    if (shouldShowHandles) {
      if (path.tool === "curve" && path.points.length === 4) {
        ...
        const isDraggingThisHandle =
          state.isDragging &&
          state.dragType === "handle" &&
          state.dragData?.pathIdx === idx;

        ctx.fillStyle = isDraggingThisHandle
          ? "rgba(255, 50, 50, 0.95)"
  ...
  if (state.ghostStroke !== false) {
    state.paths.forEach((path, idx) => {
      // Не показываем призрачный контур для выделенной фигуры
      if (idx === state.selectedPathIdx) return;

      // Проверяем видимость
      const isVisible = hasVisibleAppearance(path);

      // ОТЛАДКА - удалить после исправления
      if (path.tool === "rectangle" && idx === state.paths.length - 1) {
        console.log("Новая фигура:", {
          idx,
          fillType: path.fillType,
          thickness: path.thickness,
          isVisible: isVisible,
          willDrawGhost: !isVisible,
        });
      }
...
// 2. Создание новой кривой с двумя ручками
function createBezierCurve(start, end) {
  const dx = end.x - start.x;
  const dy = end.y - start.y;

  // Ручки на расстоянии 1/3 от начала и конца
  return {
    tool: "curve",
    points: [
      { x: start.x, y: start.y }, // начало
      { x: start.x + dx * 0.33, y: start.y + dy * 0.33 }, // первая ручка
      { x: start.x + dx * 0.66, y: start.y + dy * 0.66 }, // вторая ручка
      { x: end.x, y: end.y }, // конец
    ],
    thickness: state.thickness,
    color: state.color,
  };
...
// 3. Поиск ручек (для выделения и перетаскивания)
function findHandle(x, y) {
  ...
  for (let i = 0; i < state.paths.length; i++) {
    ...
    // ДЛЯ НОВЫХ КРИВЫХ (4 точки - Безье с двумя ручками)
    if (path.tool === "curve" && path.points.length === 4) {
      // Проверяем первую ручку (красная, индекс 1)
      const handle1 = path.points[1];
      const dist1 = Math.hypot(handle1.x - x, handle1.y - y);
      if (dist1 < threshold) {
        return { pathIdx: i, handleIdx: 1 };
      }

      // Проверяем вторую ручку (синяя, индекс 2)
      const handle2 = path.points[2];
      const dist2 = Math.hypot(handle2.x - x, handle2.y - y);
      if (dist2 < threshold) {
        return { pathIdx: i, handleIdx: 2 };
      }

      // Проверяем начальную точку (жёлтая, индекс 0)
      const start = path.points[0];
      if (Math.hypot(start.x - x, start.y - y) < threshold) {
        return { pathIdx: i, pointIdx: 0 };
      }

      // Проверяем конечную точку (жёлтая, индекс 3)
      const end = path.points[3];
      if (Math.hypot(end.x - x, end.y - y) < threshold) {
        return { pathIdx: i, pointIdx: 3 };
      }
    }

    // ДЛЯ СТАРЫХ КРИВЫХ (3 точки - квадратичные)
    if (path.tool === "curve" && path.points.length === 3) {
      const handle = path.points[1];
      if (Math.hypot(handle.x - x, handle.y - y) < threshold) {
        return { pathIdx: i, handleIdx: 1, isQuadratic: true };
      }
    }
  }
  return null;
...
// 4. Отрисовка ручек и вспомогательных линий
function drawHandles(ctx, path, idx) {
  if (path.tool === "curve" && path.points.length === 4) {
    ...
    const handleSize = Math.max(5, pointSize + 1); // Ручки чуть больше точек

    const isDraggingHandle1 =
      state.isDragging &&
      state.dragType === "handle" &&
      state.dragData?.pathIdx === idx &&
      state.dragData?.handleIdx === 1;

    const isDraggingHandle2 =
      state.isDragging &&
      state.dragType === "handle" &&
      state.dragData?.pathIdx === idx &&
      state.dragData?.handleIdx === 2;
    ...
    // ========== РУЧКИ С МАКСИМАЛЬНОЙ ВИДИМОСТЬЮ ==========
    const drawHandle = (x, y, size, isDragging, isFirstHandle) => {
      // Слой 1: Большая белая подложка
      ...
      // Слой 3: Белая внутренняя точка (блик)
      ctx.fillStyle = "#ffffff";
      ctx.beginPath();
    ...
    // Рисуем ручки
    drawHandle(handle1.x, handle1.y, handleSize, isDraggingHandle1, true);
    drawHandle(handle2.x, handle2.y, handleSize, isDraggingHandle2, false);
...
function drawTriangularGrid() {
  ...
  const side = state.triGridSize; // Длина стороны треугольника
  const height = (side * Math.sqrt(3)) / 2; // Высота треугольника
...
// Привязка к треугольной сетке
function snapToTriangular(x, y) {
  const side = state.triGridSize;
  const height = (side * Math.sqrt(3)) / 2;

  // Находим ближайший узел треугольной сетки
  ...
  let snapX = col * side + offsetX;
  let snapY = row * height;
...
function drawIsometricGrid() {
  const size = state.triGridSize;
  const angleRad = ((state.isoAngle || 30) * Math.PI) / 180; // 30 градусов по умолчанию
  ...
  // Направления осей изометрии
  // Ось X (вправо-вниз)
  const dx1 = Math.cos(angleRad);
  const dy1 = Math.sin(angleRad);
  // Ось Y (влево-вниз)
  const dx2 = Math.cos(-angleRad);
  const dy2 = Math.sin(-angleRad);
  ...
  // Рисуем линии параллельные осям
  const directions = [
    { dx: dx1, dy: dy1 }, // Ось X
    { dx: dx2, dy: dy2 }, // Ось Y
    { dx: dx3, dy: dy3 }, // Ось Z
  ...
  for (let dirIdx = 0; dirIdx < directions.length; dirIdx++) {
    ...
    // Перпендикулярное направление для смещения линий
    const perpX = -dir.dy;
    const perpY = dir.dx;
    ...
    for (let i = -lines; i <= lines; i++) {
      ...
      // Обрезаем линии по границам канваса
...
// ========== ИСТОРИЯ ДЕЙСТВИЙ (ПОЛНОСТЬЮ ИСПРАВЛЕНА) ==========
...
function initHistory() {
  ...
  console.log("✅ История инициализирована, шагов:", state.history.length);
}
...
function saveState() {
  ...
  console.log("💾 Состояние сохранено, всего шагов:", state.history.length);
}

// Отмена действия
function undo() {
  console.log("⏪ Undo вызван, история:", state.history.length);

  if (state.history.length <= 1) {
    console.log("⚠️ Нечего отменять");
    return false;
  }
  ...
  console.log("✅ Отмена выполнена, осталось шагов:", state.history.length);
  return true;
}
...
// Обновление UI фона (вынесено в отдельную функцию)
function updateBackgroundUI() {
  if (bgType) bgType.value = state.background.type;
  if (bgColor) bgColor.value = state.background.solidColor;
  if (bgColorPreview)
    bgColorPreview.style.background = state.background.solidColor;
  if (gradientColor1) gradientColor1.value = state.background.gradient.color1;
  if (gradientColor1Preview)
    gradientColor1Preview.style.background = state.background.gradient.color1;
  if (gradientColor2) gradientColor2.value = state.background.gradient.color2;
  if (gradientColor2Preview)
    gradientColor2Preview.style.background = state.background.gradient.color2;
  ...
  // Показываем/скрываем контролы фона
  if (solidBgControl)
    solidBgControl.style.display =
      state.background.type === "solid" ? "flex" : "none";
  if (gradientBgControl)
    gradientBgControl.style.display =
      state.background.type === "gradient" ? "block" : "none";
}
...
function exportSVG() {
  ...
  state.paths.forEach((path) => {
    ...
    if (path.tool === "curve" && path.points.length === 4) {
      const handle1 = path.points[1];
      const handle2 = path.points[2];
      minX = Math.min(minX, handle1.x - lineOffset, handle2.x - lineOffset);
      minY = Math.min(minY, handle1.y - lineOffset, handle2.y - lineOffset);
      maxX = Math.max(maxX, handle1.x + lineOffset, handle2.x + lineOffset);
      maxY = Math.max(maxY, handle1.y + lineOffset, handle2.y + lineOffset);
    }

    if (path.tool === "curve" && path.points.length === 3) {
      const controlPoint = path.points[1];
      minX = Math.min(minX, controlPoint.x - lineOffset);
      minY = Math.min(minY, controlPoint.y - lineOffset);
      maxX = Math.max(maxX, controlPoint.x + lineOffset);
      maxY = Math.max(maxY, controlPoint.y + lineOffset);
    }
  });
  ...
  state.paths.forEach((path, idx) => {
    ...
    if (path.tool === "circle" && path.points.length === 2) {
      ...
      paths += `<path d="M${points} Z" stroke="${pathColor}" stroke-width="${pathThickness}" ${fillAttr} stroke-linecap="round" stroke-linejoin="round"/>\n`;
    } else if (path.tool === "line" && path.points.length === 2) {
      const points = path.points.map(
        (p) => `${p.x + offsetX},${p.y + offsetY}`,
      );
      paths += `<path d="M${points[0]} L${points[1]}" stroke="${pathColor}" stroke-width="${pathThickness}" fill="none" stroke-linecap="round"/>\n`;
    } else if (path.tool === "curve" && path.points.length === 4) {
...
function exportPNG() {
  ...
  const baseWidth = Math.ceil(maxX - minX);
  const baseHeight = Math.ceil(maxY - minY);
  const offsetX = -minX;
  const offsetY = -minY;

  const scale = prompt(
    "Выберите масштаб для PNG экспорта:\n\n" +
      "1 - Обычный размер (1x)\n" +
  ...
  const finalWidth = baseWidth * selectedScale;
  const finalHeight = baseHeight * selectedScale;
...
// В функции onKeyDown, убедитесь, что этот код есть:
...
document.querySelectorAll("[data-tool]").forEach((btn) => {
  btn.onclick = () => {
    ...
    // НЕ сбрасываем thickness и color!
    draw();
  };
});
...
const fillGradientColor1Preview = document.getElementById(
  "fillGradientColor1Preview",
);
const fillGradientColor2 = document.getElementById("fillGradientColor2");
const fillGradientColor2Preview = document.getElementById(
  "fillGradientColor2Preview",
);
...
function updateFillUI() {
  ...
  gradientFillControl.style.display =
    state.fillType === "gradient" ? "block" : "none";
}
...
if (fillType) {
  fillType.onchange = (e) => {
    ...
    if (state.selectedPathIdx !== -1 && state.paths[state.selectedPathIdx]) {
      ...
      state.paths[state.selectedPathIdx].fillGradient = JSON.parse(
        JSON.stringify(state.fillGradient),
      );
      saveState();
    }
...
if (fillColorPreview) {
  fillColorPreview.onclick = () => fillColor.click();
}
...
if (fillGradientColor1 && fillGradientColor1Preview) {
  fillGradientColor1.onchange = (e) => {
    ...
    if (state.selectedPathIdx !== -1 && state.paths[state.selectedPathIdx]) {
      state.paths[state.selectedPathIdx].fillGradient.color1 =
        state.fillGradient.color1;
      saveState();
    }
...
if (fillGradientColor2 && fillGradientColor2Preview) {
  fillGradientColor2.onchange = (e) => {
    ...
    if (state.selectedPathIdx !== -1 && state.paths[state.selectedPathIdx]) {
      state.paths[state.selectedPathIdx].fillGradient.color2 =
        state.fillGradient.color2;
      saveState();
    }
...
if (fillGradientType) {
  fillGradientType.onchange = (e) => {
    ...
    if (state.selectedPathIdx !== -1 && state.paths[state.selectedPathIdx]) {
      state.paths[state.selectedPathIdx].fillGradient.type =
        state.fillGradient.type;
      saveState();
    }
...
if (fillGradientAngle) {
  fillGradientAngle.oninput = (e) => {
    ...
    if (state.selectedPathIdx !== -1 && state.paths[state.selectedPathIdx]) {
      state.paths[state.selectedPathIdx].fillGradient.angle =
        state.fillGradient.angle;
      saveState();
    }
...
if (bgColorPreview) {
  bgColorPreview.onclick = () => bgColor.click();
}

// Обработчики для градиента
...
function parseSVGToPaths(svgString) {
  ...
  // Масштабируем под наш канвас 512×512
  ...
  // Парсим все path элементы
  const pathElements = svgDoc.querySelectorAll("path");
  pathElements.forEach((pathEl) => {
    const d = pathEl.getAttribute("d");
    const stroke = pathEl.getAttribute("stroke") || state.color;
    const strokeWidth =
      parseFloat(pathEl.getAttribute("stroke-width")) || state.thickness;
    ...
    for (let cmd of commands) {
      ...
      if (type === "M" || type === "m") {
        ...
        for (let i = 0; i < coords.length; i += 6) {
          const p1 = {
            x: coords[i] * scale + offsetX,
            y: coords[i + 1] * scale + offsetY,
          };
          const p2 = {
            x: coords[i + 2] * scale + offsetX,
            y: coords[i + 3] * scale + offsetY,
          };
          const p3 = {
            x: coords[i + 4] * scale + offsetX,
            y: coords[i + 5] * scale + offsetY,
          };
    ...
    if (currentPath && currentPath.points.length >= 2) {
      ...
      if (
        currentPath.points.length === 4 &&
        Math.abs(currentPath.points[0].x - currentPath.points[3].x) < 5 &&
        Math.abs(currentPath.points[0].y - currentPath.points[3].y) < 5 &&
        currentPath.points[1].x === currentPath.points[0].x
      ) {
  ...
  circleElements.forEach((circle) => {
    ...
    const strokeWidth =
      parseFloat(circle.getAttribute("stroke-width")) || state.thickness;
  ...
  rectElements.forEach((rect) => {
    ...
    const strokeWidth =
      parseFloat(rect.getAttribute("stroke-width")) || state.thickness;
...
if (importSVGBtn && svgFileInput) {
  ...
  svgFileInput.onchange = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const newPaths = parseSVGToPaths(svgString);
...
// 1. Экспорт всех размеров для веба
async function exportAllWebSizes() {
  ...
  for (const size of sizes) {
    ...
    folder.file(`icon-${size}x${size}.png`, pngBlob);
  }
...
`;
  ...
  alert(
    '✅ Favicon комплект сохранён в папку "favicon-pack" внутри ZIP архива',
  );
}

// 3. Экспорт SVG спрайта
async function exportSprite() {
  ...
  state.paths.forEach((path, idx) => {
    ...
    if (path.tool === "circle") {
      ...
      pathData = `<polygon points="${points}" stroke="${color}" stroke-width="${thickness}" fill="none"/>`;
    }
  ...
  state.paths.forEach((_, idx) => {
    css += `.icon-${idx} { mask-image: url('sprite.svg#icon-${idx}'); }\n`;
  });
...
// 4. Экспорт PNG с конкретным размером
...
// Функция экспорта PNG с конкретным размером (используется в ZIP экспорте)
...
function updateGridUI() {
  if (state.gridMode === "square") {
    ...
    if (squareSizeRow) squareSizeRow.style.display = "none";
    if (triSizeRow) triSizeRow.style.display = "flex"; // Используем тот же ползунок
  }
}

// Обработчик изменения угла изометрии
if (isoAngleInput) {
...
// Обработчик ползунка размера треугольника
if (triGridSizeInput) {
...
// Обработчик переключения режима иконки
